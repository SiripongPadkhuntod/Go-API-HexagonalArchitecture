...
func (h *FileHandler) Upload(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, response.NewErrorResponse(port.ErrCodeBadRequest, "image is required"))
		return
	}

	f, err := file.Open()
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.NewErrorResponse(port.ErrCodeInternalServer, "failed to open image"))
		return
	}
	defer f.Close()
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.NewErrorResponse(port.ErrCodeInternalServer, "failed to read image"))
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.NewErrorResponse(port.ErrCodeInternalServer, "failed to upload image"))
		return
	}

	response.RespondSuccess(c, http.StatusOK, gin.H{"url": url})
}
